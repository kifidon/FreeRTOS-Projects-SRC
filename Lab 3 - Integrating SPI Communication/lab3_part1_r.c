 ...
 *
 *  When "spi_loopback_enabled=1", there is no SPI connection in effect. The data will be echoed back to the user using the loop back mode that uses FIFO2.
 *  When "spi_loopback_enabled=0", SPI connection is in effect. The data entered by the user will loop back from spi0 to spi1 and back to spi0. The data will be then displayed on the terminal using the FIFO2.
 * -----------------------------------------------
 *
	...
	while(1){
		if(flag==1){ 						// flag is set to 1 when the user enters the end sequence on SPI1-SPI0 mode
			 ...
			 */
				...
				u8 received;
				xQueueReceive(spi_to_uart, &received, portMAX_DELAY);

				if(received == CHAR_DOLLAR) {
					break;
				}
				...
				while(XUartPs_IsTransmitFull(UART_BASE_ADDRESS) == TRUE) { //full
					vTaskDelay(1);
				}

				XUartPs_WriteReg(UART_BASE_ADDRESS, XUARTPS_FIFO_OFFSET, received);
			}
			terminateInput();
		} else {
