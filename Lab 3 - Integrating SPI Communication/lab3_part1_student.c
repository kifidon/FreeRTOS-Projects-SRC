...
static void vUartManagerTask( void *pvParameters )
{
	...
	while(1){
		if(flag==1){
			XUartPs_WriteReg(XPAR_XUARTPS_0_BASEADDR, XUARTPS_FIFO_OFFSET, dummy);
			while(1){
				// flag is set to 1 when the user enters the end sequence on SPI1-SPI0 mode
				 ...
				 *    function wi
				 *    th the appropriate parameters.
				 */
				 if (xQueueReceive(spi_data, &uart_byte, portMAX_DELAY) == pdPASS) {
					...
					// 4. Check if UART is ready to send data
					while (XUartPs_IsTransmitFull(XPAR_XUARTPS_0_BASEADDR)) {
						taskYIELD();
					}

					// 5. Send the received byte to the UART until a null character ('\0') is received
					if (received_byte != '\0') {
						XUartPs_WriteReg(XPAR_XUARTPS_0_BASEADDR, XUARTPS_FIFO_OFFSET, received_byte);
						uart_buffer[i++] = received_byte; // Store received byte in the buffer
					} else {
						// Null character indicates the end of transmission
						uart_buffer[i] = '\0'; // Add null terminator
						break;
					}
				}
			}
...
static void vSpiMainTask( void *pvParameters )
{
	...
	u8 send_buffer[TRANSFER_SIZE_IN_BYTES];
	u8 recv_buffer[TRANSFER_SIZE_IN_BYTES];
	...
	while(1){
		...
		if(active_command == 2){
			...
			/*******************************************/
				send_buffer[received_bytes] = received_from_uart;
				received_bytes++;
				if (received_bytes == TRANSFER_SIZE_IN_BYTES) {
					XSpiPs_PolledTransfer(&spiInstMain, send_buffer, recv_buffer, TRANSFER_SIZE_IN_BYTES);
					taskYIELD();
					for (int i = 0; i < TRANSFER_SIZE_IN_BYTES; i++) {
						send_to_uart = recv_buffer[i];
					    xQueueSend(spi_to_uart, &send_to_uart, 0UL);
					}
					received_bytes = 0;
...
static void vSpiSubTask( void *pvParameters )
{
	...
	while(1)
	{
		if (spi_loopback_enabled == 0 && active_command == 2)
		{
			...
			XSpiPs_PolledTransfer(&spiInstSub, NULL, &temp_store, 1);
			...
			// Update rolling buffer with valid data
			updateRollingBuffer(buffer, temp_store);


			checkTerminationSequence(sequence_flag, &temp_store);

			// If termination sequence detected, prepare and send message back
			if (*sequence_flag == 3)
			{

				str_length = snprintf(buffer, sizeof(buffer),
									  "\nNumber of bytes received over SPI: %d\nLast message byte count: %d\nTotal messages received: %d\n",
									  spi_rx_bytes, msg_bytes, message_counter);

				// Send the message back to SPI master, byte-by-byte
				for (int i = 0; i < str_length; i++)
				{
					XSpiPs_PolledTransfer(&spiInstSub, &buffer[i], NULL, 1);
				}
